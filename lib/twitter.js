...
class GyappTwitter {
  ...
  async uploadMedia(tweetData, mediaURL, retry){
    return new Promise(async(resolve, reject)=>{
      try{
        console.log('Twitter fetch media: '+mediaURL);
        ...
        console.log('Twitter fetch finish');
        ...
        stream.on('close', async () => {
          try{
            ...
            console.log('Gyazo upload error?: '+err);
            resolve();
          }
        });
        mediaRes.body.pipe(stream);
      }catch(err){
        console.log('Twitter fetch error?: '+err);
        if(retry<5){
          await this.uploadMedia(tweetData, mediaURL, retry+1);
          resolve();
        }
      }
    });
  ...
  async uploadAllMedia(tweet, tweetData, idx){
    ...
    const promise = await new Promise(async(resolve) => {
      try{
        ...
        console.log('uploadAllMedia error?: '+err);
      }
      resolve();
    })
    if(tweet.hasOwnProperty('entities') &&
        idx < tweet.entities.media.length){
